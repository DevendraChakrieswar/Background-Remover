    ...
    <div className='py-10 md:px-20 lg:px-20'>
        <div className='container mx-auto px-4'>
            ...
            <div className='grid gap-8 md:grid-cols-2 lg:grid-cols-3'>
                ...
                {plans.map((plan) => (
                    <div key={plan.id} className={`relative pt-6 p-6 ${plan.popular ? 'backdrop-blur-lg rounded-2xl' : 'border-gray-800 rounded-xl'} bg-[#1A1A1A] hover:transfrom
                    hover:translate-y-2 transition-all duration-300`}>
                        {plan.popular && (
                            <div className='absolute -top-4 left-1/2 -translate-x-1/2 rounded-full bg-purple-600 px-3 py-1 text-white text-s, font-semibold'>
                                Most Popular
                            </div>
                        )}
